            ...
            1: {
                ...
                questions: [
                    ...
                    },
                    {
                        ...
                        title: '지금 파시는 이 집이 재개발·재건축으로 새로 지어진 아파트인가요?',
                        ...
                        options: [
                            { label: '예, 재개발·재건축으로 새로 받은 아파트예요', detail: '원조합원·승계조합원 모두 포함 — 다음 화면에서 구분', value: 'yes', icon: '입' },
                            { label: '아니오, 재개발과 무관한 일반 주택이에요', detail: '청약·분양으로 산 새 아파트 포함 — 일반 양도 흐름', value: 'no', icon: '주' }
                        ]
                    },
                    {
                ...
                questions: [
                    {
                        ...
                        helper: (inputs) => {
                            if (inputs.assetCategory === 'right' && inputs.rightType === 'ticket') {
                                return '📅 분양권 취득일은 어떻게 갖게 됐는지에 따라 달라요. 청약에 "당첨"돼서 처음 받은 거면 당첨일, 다른 사람에게서 사신(전매) 거면 그 매매대금 잔금일이에요(명의를 먼저 바꿨더라도 잔금일이 기준이에요). 양도일은 분양권을 넘기며 잔금을 다 받은 날이고요. 날짜는 당첨 통지서(입주자모집공고·당첨자 발표일)나 분양권 매매계약서의 잔금 이체내역에서 확인하세요.';
                            }
                            if (inputs.assetCategory === 'right' && inputs.rightType === 'membership') {
                                return '📅 입주권(조합원 권리)은 조합원 유형에 따라 취득일을 보는 방식이 달라요. 재개발·재건축 전부터 그 집·땅을 갖고 있던 "원조합원"이라면, 보유기간은 보통 그 종전 집·땅을 처음 산 날(잔금일)부터 따져요(이사 간 날이나 계약일이 아니에요). 입주권을 돈 주고 사 오신 "승계조합원"은 그 입주권을 산 잔금일이 취득일이고요. 다만 입주권은 관리처분인가일·준공일 등 어느 시점을 기준으로 보느냐에 따라 세금 판단이 달라질 수 있어, 큰 거래라면 종전 집의 옛 매매계약서나 입주권 매매계약서 잔금 내역을 챙겨 두고 세무 상담을 받아 보시는 걸 권해요.';
                            }
                            return '📅 정확한 날짜를 모르겠다면 — 취득일·양도일은 보통 "잔금을 모두 치른 날(잔금일)"이에요. 계약서 쓴 날이나 이사 간 날(입주·전입일)이 아니라는 점만 기억하시면 돼요. 잔금일은 매매계약서의 "잔금 지급일"과 실제 돈이 오간 계좌이체 내역으로 확인하세요. 만약 잔금을 치르기 전에 등기부터 먼저 했다면, 그때는 등기사항전부증명서(등기부등본)에 찍힌 "접수일"이 기준이에요. 참고로 상속받은 집은 돌아가신 날, 증여받은 집은 증여등기 "접수일"이 취득일이에요.';
                        },
                        type: 'date_group',
                        fields: (inputs) => {
                            if (inputs.assetCategory === 'right' && inputs.rightType === 'ticket') {
